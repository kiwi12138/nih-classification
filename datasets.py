...
class XRaysTestDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        self.the_chosen, self.all_classes, self.all_classes_dict = self.choose_the_indices()
        self.new_test_df = self.test_df.iloc[self.the_chosen, :]
        print(1)

    def __getitem__(self, index):
        row = self.new_test_df.iloc[index, :]
    ...
    def choose_the_indices(self):
        ...
        # for i in tqdm(range(len(merged_df))):
        print('\nSampling the huuuge test dataset')
        for i in tqdm(list(np.random.choice(range(length), length, replace=False))):

            temp = str.split(self.test_df.iloc[i, :]['Finding Labels'], '|')
            if 'No Finding' in temp:
                continue
            # special case of ultra minority hernia. we will use all the images with 'Hernia' tagged in them.
            # if 'Hernia' in temp:
...
if __name__=='__main__':
    a=XRaysTrainDataset(r'E:\data\nih-chest')
    b = XRaysTestDataset(r'E:\data\nih-chest')
    print(len(a),print(len(b)))
    for i in range(len(b)):
        im,tgt=b[i]
        #print(im.shape)
        assert tgt.sum().item()==1